...
zile = [20, 21, 22, 23, 24, 25, 26]

for ziua in zile:
    r = requests.get(f"https://www.mai.gov.ro/informare-covid-19-grupul-de-comunicare-strategica-{ziua}-ianuarie-ora-13-00/")
    ...
    df.to_csv(f"covid_eu_ianuarie_{ziua}.csv", header=header[1:5])
    df.to_csv(f"covid_eu_ianuarie_{ziua}.xls", header=header[1:5])
